...
dictStrCustom = {
    ...
    'strSnClearGroup': '已清除[{tName}]人物卡在本群的名片记录',
    'strSnClearGlobal': '已清除[{tName}]人物卡的全局名片记录',
    'strSnClearAll': '已清除[{tName}]人物卡中的的所有名片记录（包括群名片和全局名片）',
    'strSnClearGroupAll': '已清除[{tName}]人物卡在所有群中的名片记录（全局名片保留）',
    'strSnClearNone': '[{tName}]人物卡没有目前指令能删除的名片记录',
    ...
    'strPcDelMulti': '以下{tLenSkillName}张人物卡已删除: \n{tSkillName}',
    'strPcDelPartialSuccess': '以下{tLenSkillName}张人物卡已删除: \n{tSkillName}\n同时以下{tLenFailedSkills}段内容未匹配到人物卡，已忽略: \n{tFailedSkills}',
    'strPcDelAll': '已删除全部人物卡，共{tLenSkillName}张',
    'strPcDelError': '以下内容未匹配到可删除的人物卡: \n{tFailedSkills}',
    ...
    'strPortUsage': '人物卡引继指令：\n[.port code] 生成当前账号的同骰引继码\n[.port code [平台] [user_id]] (Master/官方头像校验)为指定账号生成同骰引继码\n[.port code out ([卡名]...) (split)] 导出跨骰引继码\n[.port code out ([卡名]...) [平台] [user_id] (split)] (Master/官方头像校验)导出指定账号人物卡\n[.port code out [对方同骰码] (split)] 凭对方同骰码导出其跨骰引继码\n[.port pull 同骰码] 或 [.port pull [平台] [user_id]] 拉取整套人物卡\n[.port in 跨骰码] 导入跨骰码(可加 force/nolock)\n默认导出当前账号全部人物卡；只有写了卡名时才导出指定人物卡\n聊天直发只发送 ODC1... 引继码正文，不额外附带 [.port in] 命令前缀\n2段及以上默认发文件；追加 [split] 可强制逐段发送，且 [split] 可放在导出参数任意位置\n[user_id] 与 [平台] 可通过 [.uinfo user] 查看\n官方机器人上填写自己的 QQ 号时，可通过头像校验代替 Master\n示例: [.port code out qq 121096913 split]',
    'strPortCodeSame': '已生成同骰引继码：{tPortCode}\n有效期{tPortTTL}, 一次性\n请在新账号处发送 [.port pull {tPortCode}] 完成迁移',
    ...
    'strPortInButSameCode': '[{tPortCode}]是同骰引继码, 请使用 [.port pull {tPortCode}] 拉取',
    'strPortPullDone': '引继完成, 共{tPortCardCount}张卡：新增{tPortNewCount}张, 改名{tPortRenameCount}张, 覆盖{tPortOverwriteCount}张\n导入的卡：{tPortCardList}',
    'strPortPullSelf': '这枚引继码指向的就是当前账号, 无需迁移',
    'strPortCodeInvalid': '引继码不存在或已过期, 请重新生成',
    'strPortCardNotFound': '未找到人物卡[{tPcName}]',
    'strPortNoCard': '当前没有可导出的人物卡',
    'strPortTargetNoCard': '{tPortTarget}没有可导出或可迁移的人物卡',
    'strPortNeedMaster': '指定账号生成引继码需要Master权限',
    'strPortAvatarCheck': '正在通过头像校验 [{tName}] 与 QQ[{tPortQQ}] 是否为同一人，请稍候',
    'strPortAvatarFail': '头像校验未通过，无法确认是同一人。请改用 [.port code] 生成同骰码，或联系骰主',
    'strPortAvatarError': '头像校验失败：无法读取头像。请稍后重试，或改用 [.port code]',
    'strPortCrcError': '引继码校验失败：可能有缺字或错字, 请重新完整复制',
    'strPortVersionError': '引继码版本不支持, 请升级插件后再试',
    'strPortDictError': '引继码使用了当前插件不支持的模板压缩字典, 请升级双方插件后重新生成引继码',
    'strPortBadFormat': '这不是有效的引继码',
    'strPortInError': '导入失败：{tPortError}',
...
dictAdapterMapper = {
    'qq': {
        'onebot': {
            ...
            'milky_default': 'Milky',
            'llonebot_milky': 'LuckyLilliaBot/Milky',
            'lagrange_milky': 'LagrangeV2',
            'yogurt_milky': 'Yogurt/Milky'
        },
        'milky': {
            'default': 'Milky',
            'llonebot_default': 'LuckyLilliaBot/Milky',
            'lagrange_default': 'LagrangeV2',
            'yogurt_default': 'Yogurt/Milky'
        }
    },
    'kaiheila': {'kaiheila_link': {'default': 'KOOK', 'text': 'KOOK'}},
    'terminal': {'terminal_link': {'ff14': 'FF14'}},
}
