...
def replyCONTEXT_regGet(plugin_event: OlivOS.API.Event, tmp_reast_str: str):
    ...
    if (
        ...
        and (tmp_hash in OlivaDiceCore.crossHook.dictReplyContextReg[tmp_bothash])
            ...
            if type(tmp_data_block) == bool:
                ...
                if tmp_data_block == 'allowCommand':
                    ...
                    [tmp_reast_str_tmp, flag_is_command] = OlivaDiceCore.msgReply.msgIsCommand(
                        tmp_reast_str, OlivaDiceCore.crossHook.dictHookList['prefix']
                    )
                    if flag_is_command:
                        res = False
...
def replySET_command(plugin_event, Proc, valDict):
    ...
    if tmp_reast_str == 'show':
        ...
        if tmp_set_para != None:
            ...
            tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                dictStrCustom['strShowGroupMainDiceNone'], dictTValue
            )
        ...
        if flag_is_from_group:
            ...
            if tmp_switch_setcoc != None:
                ...
                tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                    dictStrCustom['strSetGroupTempRule'], dictTValue
                )
            else:
                OlivaDiceCore.userConfig.setUserConfigByKey(
                    userConfigKey='groupTemplate',
                ...
                tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                    dictStrCustom['strDelGroupTempRule'], dictTValue
                )
            OlivaDiceCore.userConfig.writeUserConfigByUserHash(
                userHash=OlivaDiceCore.userConfig.getUserHash(
                    userId=tmp_hag_id, userType='group', platform=tmp_user_platform
                )
            )
        else:
        ...
        if flag_is_from_group:
            ...
            if tmp_switch_setrav != None:
                ...
                tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                    dictStrCustom['strSetGroupRavRule'], dictTValue
                )
            else:
                OlivaDiceCore.userConfig.setUserConfigByKey(
                    userConfigKey='groupRavRule',
                ...
                tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                    dictStrCustom['strDelGroupRavRule'], dictTValue
                )
            OlivaDiceCore.userConfig.writeUserConfigByUserHash(
                userHash=OlivaDiceCore.userConfig.getUserHash(
                    userId=tmp_hag_id, userType='group', platform=tmp_user_platform
        ...
        if flag_is_from_group:
            ...
            if flag_settemp_mode == 'settemp':
                ...
                if tmp_templateRuleName_input == '':
                    tmp_templateRuleName_input = 'default'
            if tmp_templateName_input == '' or OlivaDiceCore.pcCard.pcCardDataCheckTemplateKey(
                tmp_templateName_input, tmp_templateRuleName_input
            ):
                if tmp_templateName_input != '':
                    tmp_templateName_input = OlivaDiceCore.pcCard.pcCardDataCheckTemplateKey(
                        tmp_templateName_input, tmp_templateRuleName_input, resMode='temp'
                    )
                    tmp_templateRuleName_input = OlivaDiceCore.pcCard.pcCardDataCheckTemplateKey(
                        tmp_templateName_input, tmp_templateRuleName_input, resMode='rule'
                    )
                ...
                if flag_settemp_mode == 'settemp':
                    tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                        dictStrCustom['strSetGroupTempError'], dictTValue
                    )
                elif flag_settemp_mode == 'setrule':
                    tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                        dictStrCustom['strSetGroupTempRuleError'], dictTValue
                    )
        else:
            tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strForGroupOnly'], dictTValue)
        replyMsg(plugin_event, tmp_reply_str)
                ...
                if tmp_set_para_rd.resError == None:
                    ...
                    if len(tmp_set_para_list) == 2:
                        if tmp_set_para_list[0].isdigit() and tmp_set_para_list[1].isdigit():
                            ...
                            tmp_set_D_left = int(tmp_set_para_list[0])
                            tmp_set_D_right = 100
                            tmp_set_para = '%sD%d' % (tmp_set_para_list[0], tmp_set_D_right)
        ...
        if tmp_set_para != None:
            ...
            dictTValue['tResult'] = tmp_set_para
            tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                dictStrCustom['strSetGroupMainDice'], dictTValue
            )
        else:
            OlivaDiceCore.userConfig.setUserConfigByKey(
                userConfigKey='groupMainDice',
            ...
            tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                dictStrCustom['strDelGroupMainDice'], dictTValue
            )
        OlivaDiceCore.userConfig.writeUserConfigByUserHash(
            userHash=OlivaDiceCore.userConfig.getUserHash(
...
def get_SkillCheckResult(
    ...
    if pcHash and pcCardName:
        ...
        if user_id and skill_name and platform and botHash and hagId:
            ...
            record_skill_check_result(user_id, pcCardName, skill_name, is_success, platform, botHash, hagId)
    if tmpSkillCheckType == OlivaDiceCore.skillCheck.resultType.SKILLCHECK_SUCCESS:
        ...
        res = OlivaDiceCore.msgCustomManager.formatReplySTR(
            dictStrCustom['strPcSkillCheckExtremeHardSucceed'], dictTValue
        )
    elif tmpSkillCheckType == OlivaDiceCore.skillCheck.resultType.SKILLCHECK_GREAT_SUCCESS:
...
def get_SkillCheckError(resError, dictStrCustom, dictTValue):
    if resError == OlivaDiceCore.onedice.RD.resErrorType.UNKNOWN_GENERATE_FATAL:
        ...
        tmp_reply_str_1 = OlivaDiceCore.msgCustomManager.formatReplySTR(
            dictStrCustom['strRollErrorUnknown'], dictTValue
        )
...
def record_skill_check_result(user_id, pc_name, skill_name, is_success, platform, botHash, hagId):
    ...
    try:
        ...
        log_status = OlivaDiceCore.userConfig.getUserConfigByKey(
            userId=hagId, userType='group', platform=platform, userConfigKey='logStatus', botHash=botHash
        )
        ...
        log_enable = OlivaDiceCore.userConfig.getUserConfigByKey(
            userId=hagId, userType='group', platform=platform, userConfigKey='logEnable', botHash=botHash
        )
        ...
        log_active_name = OlivaDiceCore.userConfig.getUserConfigByKey(
            userId=hagId, userType='group', platform=platform, userConfigKey='logActiveName', botHash=botHash
        )
        ...
        log_name_dict = OlivaDiceCore.userConfig.getUserConfigByKey(
            userId=hagId, userType='group', platform=platform, userConfigKey='logNameDict', botHash=botHash
        )
        ...
        if user_hash not in log_status[log_uuid]:
            log_status[log_uuid][user_hash] = {'id': str(user_id), '人物卡': {}}

        # 确保人物卡条目存在
        if pc_name not in log_status[log_uuid][user_hash]['人物卡']:
            log_status[log_uuid][user_hash]['人物卡'][pc_name] = {'成功': {}, '失败': {}}

        # 将技能名转换为主键名
        pc_hash = OlivaDiceCore.pcCard.getPcHash(user_id, platform)
        main_skill_name = OlivaDiceCore.pcCard.pcCardDataSkillNameMapper(
            pc_hash, skill_name, flagShow=False, hagId=hagId
        )
        ...
        if type(tmp_reast_str_para.data[-1]) == OlivOS.messageAPI.PARA.at:
            ...
            if is_rav:
                ...
                if rd_para.resError == None:
                    ...
                    dictRuleTempData = {'roll': rd_para.resInt, 'skill': tmp_skill_value_0}
                    ...
                    dictTValue['tSkillValue'] = (
                        str(tmp_skill_value_0) if not difficulty_0 else f'{tmpSkillThreshold}({str(tmp_skill_value_0)})'
                    )
                ...
                if rd_para_1.resError == None:
                    ...
                    dictRuleTempData_1 = {'roll': rd_para_1.resInt, 'skill': tmp_skill_value_1}
                    ...
                    dictTValue['tSkillValue01'] = (
                        str(tmp_skill_value_1)
                        if not difficulty_1
                        else f'{tmpSkillThreshold_1}({str(tmp_skill_value_1)})'
                    )
                flag_rav_type = 'x'
                if tmpSkillCheckType in dictSkillCheckRank and tmpSkillCheckType_1 in dictSkillCheckRank:
                    ...
                    dictTValue['tSkillCheckReasult01'] = get_SkillCheckResult(
                        ...
                        user_id=tmp_userID_1,
                        skill_name=tmp_skill_name_1,
                        platform=tmp_platform,
                        botHash=plugin_event.bot_info.hash,
                        hagId=tmp_hagID,
                    )
                ...
                if flag_rav_type == '0' and tmpSkillCheckType in [
                    ...
                    tmp_enhanceList = OlivaDiceCore.pcCard.pcCardDataGetTemplateDataByKey(
                        tmp_pcHash, tmp_pc_name_0, 'enhanceList', []
                    )
                    ...
                    if (
                        tmp_skill_name_0.upper() not in tmp_enhanceList
                        and tmp_skill_name_0.upper() not in tmp_skipEnhance_list
                    ):
                        tmp_enhanceList.append(tmp_skill_name_0.upper())
                        OlivaDiceCore.pcCard.pcCardDataSetTemplateDataByKey(
                            tmp_pcHash, tmp_pc_name_0, 'enhanceList', tmp_enhanceList
                        )
                    ...
                    if tmp_template_name is not None:
                        ...
                        if 'skillConfig' in tmp_template and 'skipEnhance' in tmp_template['skillConfig']:
                            if isinstance(tmp_template['skillConfig']['skipEnhance'], list):
                                tmp_skipEnhance_list = tmp_template['skillConfig']['skipEnhance']
                    if (
                        tmp_skill_name_1.upper() not in tmp_enhanceList
                        and tmp_skill_name_1.upper() not in tmp_skipEnhance_list
                    ):
                ...
                dictTValue['tSkillName01'] = (
                    tmp_skill_name_1 if not difficulty_1 else f'{tmp_skill_name_1}({difficulty_1})'
                )
                if flag_rav_type == '0':
                    dictTValue['tRAVResult'] = OlivaDiceCore.msgCustomManager.formatReplySTR(
                        dictStrCustom['strRAVResult01'], dictTValue
                    )
                elif flag_rav_type == '1':
                    dictTValue['tRAVResult'] = OlivaDiceCore.msgCustomManager.formatReplySTR(
                        dictStrCustom['strRAVResult02'], dictTValue
                    )
                elif flag_rav_type == '-':
                    dictTValue['tRAVResult'] = OlivaDiceCore.msgCustomManager.formatReplySTR(
                        dictStrCustom['strRAVResult03'], dictTValue
                    )
...
def setPcNoteOrRecData(
    ...
    if tmp_pc_name == None:
        ...
        if not OlivaDiceCore.pcCard.checkPcName(tmp_pc_name):
            tmp_pc_name = '用户'
        if not OlivaDiceCore.pcCard.pcCardRebase(tmp_pcHash, tmp_pc_name, tmp_hagID):
            return
    dictTValue['tName'] = tmp_pc_name
    tmp_mappingRecord = OlivaDiceCore.pcCard.pcCardDataGetTemplateDataByKey(
        pcHash=tmp_pcHash, pcCardName=tmp_pc_name, dataKey=keyName, resDefault={}
    )
    ...
    if is_remove and tmp_key is not None:
        if tmp_key in tmp_mappingRecord:
            ...
            OlivaDiceCore.pcCard.pcCardDataSetTemplateDataByKey(
                pcHash=tmp_pcHash, pcCardName=tmp_pc_name, dataKey=keyName, dataContent=tmp_mappingRecord
            )
            if enableFalse:
                if flag_mode == 'rec':
                    dictTValue['tSkillName'] = tmp_key
                    tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                        dictStrCustom['strPcRecRm'], dictTValue
                    )
                    OlivaDiceCore.msgReply.trigger_auto_sn_update(
                        plugin_event, tmp_pc_id, tmp_pc_platform, tmp_hagID, dictTValue
                    )
                    ...
                    tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                        dictStrCustom['strPcNoteRm'], dictTValue
                    )
                    OlivaDiceCore.msgReply.replyMsg(plugin_event, tmp_reply_str)
        else:
            if enableFalse:
                if flag_mode == 'rec':
                    dictTValue['tSkillName'] = tmp_key
                    tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                        dictStrCustom['strPcRecError'], dictTValue
                    )
                    OlivaDiceCore.msgReply.replyMsg(plugin_event, tmp_reply_str)
                else:
                    dictTValue['tSkillName'] = tmp_key
                    tmp_reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                        dictStrCustom['strPcNoteError'], dictTValue
                    )
                    OlivaDiceCore.msgReply.replyMsg(plugin_event, tmp_reply_str)
        return
    ...
    if tmp_key != None and tmp_value != None:
        ...
        if tmp_pcCardRule in OlivaDiceCore.pcCardData.dictPcCardMappingSpecial:
            if tmp_key.upper() in [
                skill.upper() for skill in OlivaDiceCore.pcCardData.dictPcCardMappingSpecial[tmp_pcCardRule]
            ]:
...
def replyRI_command(
    plugin_event, tmp_reast_str, tmp_pc_id, tmp_pc_platform, tmp_hagID, dictTValue, dictStrCustom, flag_reply=True
):
    # 支持多种分隔符: 逗号、分号、空格
    # 首先将所有分隔符统一替换为逗号
    ...
    for i in range(batch_count):
        ...
        if tmp_value_rd.resError == None:
            ...
            setUserConfigForInit(
                ...
                init_name=tmp_name,
                init_value={'userId': tmp_pc_id, 'platform': tmp_pc_platform},
            )
            added_count += 1
...
def setUserConfigForInit(tmp_hagID, tmp_pc_platform, bot_hash, config_key, init_name, init_value):
...
def getNoteFormat(data: str, pcHash: str, hagID):
    ...
    for skill_name in tmp_dict_pc_card:
        res = res.replace('{%s}' % skill_name, '%d' % tmp_dict_pc_card[skill_name])
    tmp_value = 99
    if '克苏鲁神话' in tmp_dict_pc_card:
        tmp_value = 99 - tmp_dict_pc_card['克苏鲁神话']
    res = res.replace('{SANMAX}', '%d' % (tmp_value))
    return res
...
def format_team_member_display(user_name, pc_name, dictStrCustom, template_key='strTeamMemberFormat', **kwargs):
    ...
    format_dict = {'tUserName': user_name, 'tPcName': display_pc_name, **kwargs}
    return OlivaDiceCore.msgCustomManager.formatReplySTR(
        dictStrCustom.get(template_key, '[{tUserName}] - [{tPcName}]'), format_dict
    )
...
def team_create(plugin_event, tmp_reast_str, tmp_hagID, dictTValue, dictStrCustom, team_name):
    ...
    if team_name not in team_config:
        team_config[team_name] = {'members': list(set(members)), 'created': int(time.time())}
        # 设置新小队为活跃小队
        OlivaDiceCore.userConfig.setUserConfigByKey(
            userConfigKey='activeTeam',
...
def team_show(plugin_event, tmp_reast_str, tmp_hagID, dictTValue, dictStrCustom, team_name):
    ...
    if not team_name:
        if active_team is None:
            OlivaDiceCore.msgReply.replyMsg(
                plugin_event,
                OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strNoActiveTeam'], dictTValue),
            )
            return
        team_name = active_team
    # 检查小队是否存在
    if team_name not in team_config:
        dictTValue['tTeamName'] = team_name
        OlivaDiceCore.msgReply.replyMsg(
            plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamNotFound'], dictTValue)
        )
        return
    # 获取成员信息
    members = team_config[team_name]['members']
    if not members:
        dictTValue['tTeamName'] = team_name
        OlivaDiceCore.msgReply.replyMsg(
            plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamEmpty'], dictTValue)
        )
        return
    member_info = []
    index = 1
    ...
    dictTValue['tMembers'] = '\n'.join(member_info)
    OlivaDiceCore.msgReply.replyMsg(
        plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamShow'], dictTValue)
    )
...
def team_list(plugin_event, tmp_hagID, dictTValue, dictStrCustom):
    ...
    if not team_list:
        OlivaDiceCore.msgReply.replyMsg(
            plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strNoTeams'], dictTValue)
        )
        return
    dictTValue['tTeamCount'] = str(len(team_list))
    dictTValue['tTeamList'] = '\n'.join(team_list)
    OlivaDiceCore.msgReply.replyMsg(
        plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamList'], dictTValue)
    )
...
def team_remove(
    ...
    if not removed_members:
        OlivaDiceCore.msgReply.replyMsg(
            plugin_event,
            OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strNoMembersRemoved'], dictTValue),
        )
        return
    removed_names = []
    for member_id in removed_members:
        ...
        pc_name = OlivaDiceCore.pcCard.pcCardDataGetSelectionKey(pc_hash, tmp_hagID)
        display_name = (
            format_team_member_display(user_name, pc_name, dictStrCustom, 'strTeamSkillUpdateMemberFormat')
            .replace('->', '')
            .replace(':', '')
        )
    ...
    dictTValue['tMembers'] = '\n'.join(member_info)
    OlivaDiceCore.msgReply.replyMsg(
        plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strMembersRemoved'], dictTValue)
    )
...
def team_delete(
    ...
    dictTValue['tTeamName'] = team_name
    OlivaDiceCore.msgReply.replyMsg(
        plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamDeleted'], dictTValue)
    )
...
def team_clear(
    ...
    dictTValue['tTeamName'] = team_name
    OlivaDiceCore.msgReply.replyMsg(
        plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamCleared'], dictTValue)
    )
...
def team_at(plugin_event, tmp_reast_str, tmp_hagID, dictTValue, dictStrCustom, team_name):
    ...
    reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamAt'], dictTValue)
...
def team_set(plugin_event, tmp_reast_str, tmp_hagID, dictTValue, dictStrCustom, team_name):
    ...
    if team_name in team_config:
        ...
        dictTValue['tTeamName'] = team_name
        OlivaDiceCore.msgReply.replyMsg(
            plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamSetActive'], dictTValue)
        )
    else:
        # 如果直接切换失败，尝试模糊搜索
        ...
        if len(team_list) > 0:
            ...
            if selected_team_name is not None:
                ...
                OlivaDiceCore.msgReply.replyMsg(
                    plugin_event,
                    OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamSetActive'], dictTValue),
                )
        else:
            # 如果没有任何小队，显示错误
            dictTValue['tTeamName'] = team_name
            OlivaDiceCore.msgReply.replyMsg(
                plugin_event,
                OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamNotFound'], dictTValue),
            )
...
def team_sort(plugin_event, tmp_reast_str, tmp_hagID, dictTValue, dictStrCustom, team_name):
    ...
    if not skill_name:
        OlivaDiceCore.msgReply.replyMsg(
            plugin_event,
            OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamSortNeedSkill'], dictTValue),
        )
        return
    ...
    for original_index, member_id in enumerate(members):
        ...
        skill_value = 0
        if pc_name:
            skill_value = OlivaDiceCore.pcCard.pcCardDataGetBySkillName(pc_hash, skill_name, hagId=tmp_hagID)
    ...
    reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamSortResult'], dictTValue)
    OlivaDiceCore.msgReply.replyMsg(plugin_event, reply_str)
...
def team_rename(plugin_event, tmp_reast_str, tmp_hagID, dictTValue, dictStrCustom):
    ...
    if not new_name:
        OlivaDiceCore.msgReply.replyMsg(
            plugin_event,
            OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamRenameNeedNewName'], dictTValue),
        )
        return
    if not old_name:
        OlivaDiceCore.msgReply.replyMsg(
            plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strNoActiveTeam'], dictTValue)
        )
        return
    # 检查旧小队是否存在
    ...
    if new_name in team_config:
        ...
        OlivaDiceCore.msgReply.replyMsg(
            plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamNameExists'], dictTValue)
        )
        return
    ...
    OlivaDiceCore.msgReply.replyMsg(
        plugin_event, OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamRenamed'], dictTValue)
    )
...
def team_st(plugin_event, tmp_reast_str, tmp_hagID, dictTValue, dictStrCustom, team_name):
                ...
                for skill in sorted(all_pc_skill_names, key=len, reverse=True):
                    if skill.startswith(start_char.upper()) and processed_skill_ops[i:].upper().startswith(skill):
                        ...
                        if (
                            start_char.upper() == 'D'
                            and len(skill) == 1
                            and i + 1 < len(processed_skill_ops)
                            and processed_skill_ops[i + 1].isdigit()
                        ):
                            i += 1
                            break
    ...
    while current_pos < len(processed_skill_ops):
        ...
        if processed_skill_ops[skill_end_pos].isdigit() or processed_skill_ops[skill_end_pos] in all_op_list:
            ...
            if (
                ...
                and skill_end_pos < len(processed_skill_ops)
                and processed_skill_ops[skill_end_pos].isdigit()
            ):
    ...
    for member_id in members:
            ...
            if rd_para.resError is None:
                ...
                if op != '' or 'D' in expr_str.upper() or has_calc_in_expr:
                    if 'D' in expr_str.upper():
                        ...
                        if len(detail_str) > 50:
                            ...
                            skill_update_detail = (
                                f'{current_value} -> {new_value} ({calculation_detail}={rd_para.resDetail})'
                            )
                    else:
                        # 普通计算表达式
                ...
                member_results.append(
                    OlivaDiceCore.msgCustomManager.formatReplySTR(
                        ...
                        {
                            'tSkillName': skill_name,
                            'tOldValue': current_value,
                            'tNewValue': new_value,
                            'tDetail': skill_update_detail,
                        },
                    )
                )
            else:
        ...
        user_name = get_user_name(plugin_event, member_id)
        if member_results:
            member_header = format_team_member_display(
                user_name, tmp_pc_name, dictStrCustom, 'strTeamSkillUpdateMemberFormat'
            )
    ...
    if special_skills:
        ...
        special_notice = ''
    reply_msg = (
        OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamSkillUpdate'], dictTValue) + special_notice
    )
...
def team_ra(plugin_event, tmp_reast_str, tmp_hagID, dictTValue, dictStrCustom, team_name):
    ...
    if skill_expr == '' or skill_expr is None:
        OlivaDiceCore.msgReply.replyMsg(
            plugin_event,
            OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamCheckResultNone'], dictTValue),
        )
        return
    # 解析难度前缀
    ...
    for member_id in members:
        ...
        if skill_name:
            ...
            base_skill_value = OlivaDiceCore.pcCard.pcCardDataGetBySkillName(pc_hash, skill_name_upper, hagId=tmp_hagID)
        ...
        skill_check_data = {'roll': roll_value, 'skill': current_skill_value}
        ...
        if skill_name:
            result_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
                dictStrCustom.get(
                    'strTeamCheckMemberFormat',
                    '{tDisplayName}({tSkillName}: {tSkillValue}): {tDiceDetail}/{tSkillValue} {tResult}',
                ),
                {
        ...
        if skill_check_type in [
            ...
            if pc_name and skill_name and bp_type != 1:
                tmp_pcHash = OlivaDiceCore.pcCard.getPcHash(member_id, plugin_event.platform['platform'])
                tmp_enhanceList = OlivaDiceCore.pcCard.pcCardDataGetTemplateDataByKey(
                    tmp_pcHash, pc_name, 'enhanceList', []
                )
                tmp_skipEnhance_list = []
                if template_name:
                ...
                if tmp_skill_name_core not in tmp_enhanceList and tmp_skill_name_core not in tmp_skipEnhance_list:
                    ...
                    OlivaDiceCore.pcCard.pcCardDataSetTemplateDataByKey(
                        tmp_pcHash, pc_name, 'enhanceList', tmp_enhanceList
                    )
        # 记录结果
        success_level = 0
        if skill_check_type in [
    ...
    # 没有结果
    if not results:
        reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamEmpty'], dictTValue)
        OlivaDiceCore.msgReply.replyMsg(plugin_event, reply_str)
        return
    ...
    dictTValue['tTeamName'] = team_name
    dictTValue['tResult'] = '\n'.join(result_lines)
    reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamCheckResult'], dictTValue)
    OlivaDiceCore.msgReply.replyMsg(plugin_event, reply_str)
...
def team_sc(plugin_event, tmp_reast_str, tmp_hagID, dictTValue, dictStrCustom, team_name):
    ...
    for member_id in members:
        ...
        # 构建显示名称
        display_name = format_team_member_display(user_name, pc_name, dictStrCustom, 'strTeamMemberFormat')
        current_san = OlivaDiceCore.pcCard.pcCardDataGetBySkillName(pc_hash, 'SAN', hagId=tmp_hagID)
        ...
        OlivaDiceCore.pcCard.pcCardDataSetBySkillName(pc_hash, 'SAN', new_san, pc_name, hagId=tmp_hagID)
        ...
        if is_mh_enabled and is_mythic_hardened and original_san_loss > 0 and san_loss != original_san_loss:
            mh_dict = {'tOriginalLoss': str(original_san_loss), 'tActualLoss': str(san_loss)}
    ...
    dictTValue['tTeamName'] = team_name
    dictTValue['tResult'] = '\n'.join(result_lines)
    reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamSCResult'], dictTValue)
    OlivaDiceCore.msgReply.replyMsg(plugin_event, reply_str)
...
def team_r(plugin_event, tmp_reast_str, tmp_hagID, dictTValue, dictStrCustom, team_name):
    ...
    for member_id in members:
        ...
        if pc_name is not None:
            skill_valueTable.update(
                OlivaDiceCore.pcCard.pcCardDataGetTemplateDataByKey(
                    pcHash=pc_hash, pcCardName=pc_name, dataKey='mappingRecord', resDefault={}
                )
            )
        rd_para_str = '1D100'
        ...
        if rd_para.resError is None:
            if not current_rd_para_str_show:
                current_rd_para_str_show = current_rd_para_str
            if not any(
                op in current_rd_para_str_show for op in ['+', '-', '*', '/', '^']
            ) and current_rd_para_str.endswith('D'):
                current_rd_para_str_show += str(rd_para_main_D_right)
            tmp_resDetail_str = OlivaDiceCore.onediceOverride.RDDataFormat(data=rd_para.resDetailData, mode='default')
    ...
    for i, result in enumerate(roll_results, 1):
        member_roll_str = OlivaDiceCore.msgCustomManager.formatReplySTR(
            dictStrCustom.get('strTeamRollMemberFormat', '[{tUserName}] - [{tPcName}]: {tRollResult}'),
            {
                ...
                'tRollDetail': result['roll_result'].split('=')[0]
                if '=' in result['roll_result']
                else result['roll_result'],
            },
        )
    ...
    if rd_reason_str:
        ...
        reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamRollWithReason'], dictTValue)
    else:
        reply_str = OlivaDiceCore.msgCustomManager.formatReplySTR(dictStrCustom['strTeamRoll'], dictTValue)
    OlivaDiceCore.msgReply.replyMsg(plugin_event, reply_str)
