...
st中的技能录入，技能修改，人物卡录入，技能展示，显示人物卡指令可由管理员及以上权限的人代他人执行。执行方式：在指令的末尾@对方''',
...
.ra(b/p)[技能名] [技能值]    奖惩检定
.ra[技能名](+-*/)[运算数] [技能值]    快速检定
.rav [技能名1] (数值1) (技能名2) (数值2) [@对方] 与指定用户进行对抗检定(带小括号的非必须且必须有空格)
技能名可带有难度(困难，极难，大成功)，对应检定数值也会按照规则进行设定
ra指令可由管理员及以上权限的人代他人执行。执行方式：在指令的末尾@对方''',

    'sc': '''理智检定
.sc [成功表达式]/[失败表达式] [san值]   理智检定
sc指令可由管理员及以上权限的人代他人执行。执行方式：在指令的末尾@对方''',
...
.sn dnd  套用dnd跑团名片
.sn [自定义名片]  套用任意自定义名片
sn指令中除自动群名片以外的指令均可由管理员及以上权限的人代他人执行。执行方式：在指令的末尾@对方''',
