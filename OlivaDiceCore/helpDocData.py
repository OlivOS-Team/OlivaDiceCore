...
.bot on 启用指令
.bot off 停用指令""",
    'port': """人物卡引继 - 一条命令带走你的全部人物卡
(卡内容、模板、房规、成长列表、骰点统计、锁定群号等)
...
参数说明:
1. 导出范围参数：
    [卡名]...    指定多张人物卡；只有写了卡名才会只导出这些人物卡；只要有不存在的卡名，就不会生成
2. 指定账号参数可省略，建议放在最后：
    [平台] [user_id] / [user_id] [平台]
   user_id 与平台可通过 [.uinfo user] 查看
3. 发送方式参数：
    不写    单段时直接回复正文；2段及以上默认整理成文件上传
    split    强制不发文件，改为逐段发送；可放在导出参数任意位置
...
.port in nolock [码]    不恢复群锁定信息
多段跨骰码导入时，系统会按同一批次的段数自动收集；收齐全部分段后立刻开始导入""",
...
- 成功率：成功率百分比（精确到两位小数）""",
    'mh': """神话淬炼/精神固化
.mh / .mh on / .mh off    开关控制
.mh list    查看处于神话淬炼状态的人物卡列表
.mh add [人物卡名]    手动添加人物卡到神话淬炼状态
.mh del [人物卡名]    手动移除人物卡的神话淬炼状态
.mh clear    清空所有神话淬炼状态

【神话淬炼机制说明】
此为可选规则
当调查员的克苏鲁神话技能点数超过理智值(SAN)时，他将经历范式转移。
这名调查员的精神经受了恐怖之物的历炼，不再轻易被撼动。

触发条件: 克苏鲁神话技能点数 > 理智值(SAN)
效果: 所有理智损失减半并向上取整
只有当本群开启神话淬炼效果时，理智减半才会生效

注意: 神话淬炼是永久性的转变，即使理智值再次超过克苏鲁神话技能也不会退回原状""",
    '疯狂症状': """疯狂症状：
.ti 临时疯狂症状
.li 总结疯狂症状""",
    'nn': """命名
...
.sn clear/clr 清除名片记录（自动判断群/全局）
.sn group clr 清除当前群的名片记录
.sn group clr all 清除所有群的名片记录（保留全局名片）
.sn global clr 清除全局名片记录
...
sn指令中除自动群名片的和清除群名片的指令均可由管理员及以上权限的人代骰。
代骰方式：在指令的末尾@目标用户""",
    'rav': """对抗检定：
.rav [技能名1] (数值1) (技能名2) (数值2) [@对方] 与指定用户进行对抗检定
